...
// 	}
	function getColumnValue($colName, $row)
	{
		...
		{
			if ($value == "")  {
				throw new Exception("Pflichtfeld in Zeile xxx Spalte " .$colName);
			}
		}
		...
		elseif ($colName == "N") {
			if (!preg_match("/^\d{1,2}\.\d{1,2}\.\d{4}$/", $value))
			{
				throw new Exception("Datumsformat in Zeile XXX Spalte $colname pr¸fen");
			}
		}
		...
		elseif ($colName == "D" || $colName == "E") {
			...
			if (preg_match("[\d]", $value) )
			{
				throw new Exception("Textformatierung in Zeile XXX Spalte $colName pr¸fen");
			}
		}
